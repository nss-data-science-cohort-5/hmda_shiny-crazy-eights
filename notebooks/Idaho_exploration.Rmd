...
```{r}
# TO DO - Table and visual showing select LEI compared to selected 
# geographical area (County, State, or MSA/Census Tract)
```
...
```{r}
# SUMMARY STATISTICS ABOUT LENDERS - LEI IS MISSING FOR MANY LOANS
idaho_2020 %>%
  group_by(lei) %>%
...
```{r}
# BONUS / TO-DO - LOANS APPROVED OR DENIED BY LENDER - pivot_longer OR pivot_wider?
idaho_approval_counts <- idaho_2020 %>%
  group_by(lei) %>%
  count(derived_race) %>%
  mutate(race_pct = (n * 100) / sum(n))

idaho_approval_counts
```	
